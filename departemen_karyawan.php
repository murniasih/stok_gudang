...
<body>
   ...
   <div class="col-md-10" style="min-height:500px">
         <div class="col-md-12" style="padding:10px; padding-left:0;padding-right:0;">
            <a href="tambahdepartemen.php" class="btn btn-info">Tambah</a>
         </div>
		 
            <table class="table table-striped">
               <tr>
                  <th class="info"><center>Nomor</center></th>
                  <th class="info"><center>Kode Departemen</center></th>
				  <th class="info"><center>Nama Departemen</center></th>
				  <th class="info"><center>PIC</center></th>
		...
		if(mysqli_num_rows($result) == 0){	//ini artinya jika data hasil query di atas kosong
			...
			while($data = mysqli_fetch_assoc($result)){	//perulangan while dg membuat variabel $data yang akan mengambil data di database
				...
				echo '<tr>';
					...
					echo '<td>'.$data['kode_departemen'].'</td>';	//menampilkan data nama lengkap dari database
					echo '<td>'.$data['nama_departemen'].'</td>';	//menampilkan data kelas dari database
					echo '<td>'.$data['pic'].'</td>';	//menampilkan data nama lengkap dari database
					echo '<td><a href="edit_departemen.php? id_departemen='.$data['id_departemen'].'" class="fa fa-edit fa-fw"></a> &nbsp / <a href="hapus_departemen.php? id_departemen='.$data['id_departemen'].'" class="fa fa-trash-o fa-fw" onclick="return confirm(\'Apakah Data Akan dihapus??\')"></a></td>';	//menampilkan link edit dan hapus dimana tiap link terdapat GET id -> ?id=siswa_id
				echo '</tr>';
