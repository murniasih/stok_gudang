...
}else if( strlen($pasword) < 2 )
{
	...
	$pasword = $conn->escape_string($pasword);
	...
	# di inputkan
	$sql = "SELECT nama, hak_akses FROM tabel_user 
			WHERE username='$username' 
			AND pasword='$pasword' LIMIT 1";
	...
	if( !$query )
	{
		die( 'Oops!! Database gagal '. $conn->error );
	}
